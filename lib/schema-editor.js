...
import { guessTagFromHeader as guessTag, normalizeDropdownOptions } from './field-mapper.js';
...
export function restoreTabSchema(tab, snapshot) {
  ...
  tab.mappings = snapshot.mappings.map((m) => ({ ...m, dropdownOptions: [...(m.dropdownOptions || [])] }));
...
export function applySchemaChange(headers, mappings, change) {
  ...
  switch (change.op) {
    ...
    case SCHEMA_OPS.SET_DROPDOWN_OPTIONS: {
      const { columnIndex, options } = change;
      const m = nextMappings.find((x) => x.columnIndex === columnIndex);
      if (m) {
