...
export function resolveFieldTag(mapping) {
  ...
  const tag = mapping.tag || FIELD_TAGS.CUSTOM_TEXT;
...
export function buildMappingsFromHeaders(headers) {
  return headers.map((header, index) => ({
    ...
    tag: header ? guessTagFromHeader(header) : FIELD_TAGS.IGNORE,
    dropdownOptions: [],
  }));
}
...
export function enrichMappingsWithDropdownOptions(mappings, rows) {
  return mappings.map((mapping) => {
    ...
    return {
      ...mapping,
      dropdownOptions: mergeDropdownOptions(configured, fromSheet),
    };
  });
}
