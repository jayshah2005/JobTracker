...
export function buildDefaultMappings() {
  return DEFAULT_HEADERS.map((header, index) => {
    ...
    return {
      ...
      header,
      tag,
      dropdownOptions: seedDropdownOptionsForHeader(header, tag),
    };
  });
}
...
export function buildMappingsFromHeaders(headers) {
  return headers.map((header, index) => {
    ...
    return {
      ...
      header: name,
      tag,
      dropdownOptions: seedDropdownOptionsForHeader(name, tag),
    };
  });
}
  ...
  for (const mapping of mappings) {
    ...
    if (tag === FIELD_TAGS.CUSTOM_DROPDOWN) {
      ...
      const defaultStatus =
        ...
        extractedData.applicationStatus ||
        'Applied';
      const suggestedValue = isApplicationStatusHeader(header)
        ? defaultStatus
        : '';
      needsInput.push({
        type: 'dropdown',
