...
/**
 * Normalize any common sheet/UI date string to YYYY-MM-DD for <input type="date">
 * and Sheets writes. Returns '' if unparseable.
 */
export function toDateInputValue(raw) {
  ...
  // M/D/YYYY or MM/DD/YYYY (US sheets)
  ...
  // D/M/YYYY when day > 12 (unambiguous)
  const dm = s.match(/^(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{4})$/);
  if (dm && Number(dm[1]) > 12 && Number(dm[2]) >= 1 && Number(dm[2]) <= 12) {
  ...
  if (!Number.isNaN(parsed)) {
    ...
    if (y > 1990 && y < 2100) return formatDate(d);
  }
...
export function hyperlinkLabel(value) {
  ...
  if (!raw) return '';
  const match = parseHyperlinkFormula(raw);
  if (!match) return raw;
  return match.label;
}

/**
 * URL from a HYPERLINK formula cell, or empty when the cell is plain text.
 */
export function hyperlinkUrl(value) {
  const match = parseHyperlinkFormula(value);
  return match?.url || '';
}
  ...
  for (const mapping of mappings) {
    ...
    switch (tag) {
      ...
      case FIELD_TAGS.CURRENT_DATE: {
        // Never let a blank form cell wipe today's date. Empty col_ keys are
        // common when <input type="date"> fails to display a non-ISO value.
        const raw = firstNonEmpty(
          userInputs[`col_${columnIndex}`],
          userInputs[tag],
          userInputs[mapping.tag]
        );
        value = toDateInputValue(raw) || formatDate();
        break;
      }
      case FIELD_TAGS.ID:
      ...
      case FIELD_TAGS.JOB_POSTED_DATE: {
        ...
        value = toDateInputValue(raw) || raw || '';
        break;
      }
    ...
    // Date tags already resolved above — do not let an empty col_ wipe them.
    if (
    ...
    // Keep a canonical posting URL even if the form URL field was cleared —
    // duplicate detection depends on it after company/role edits.
    if (tag === FIELD_TAGS.URL && !String(value).trim()) {
      value = jobUrl || String(extractedData.url || '').trim();
    }
    ...
    if (
      ...
      value =
        ...
        (isApplicationStatusHeader(mapping.header)
          ? extractedData.applicationStatus || 'Applied'
          : '');
    }
...
/**
 * Convert a saved sheet row into sidepanel form inputs (col_ / dropdown_ keys).
 */
export function rowToFormInputs(mappings = [], row = []) {
  const inputs = {};
  for (const mapping of mappings || []) {
    const tag = resolveFieldTag(mapping);
    ...
    const raw = row?.[col];
    let value = '';
    if (tag === FIELD_TAGS.URL) {
      value = hyperlinkUrl(raw) || String(raw ?? '').trim();
    } else if (tag === FIELD_TAGS.ROLE) {
      value = hyperlinkLabel(raw);
    } else if (
      tag === FIELD_TAGS.DATE_APPLIED ||
      tag === FIELD_TAGS.CURRENT_DATE ||
      tag === FIELD_TAGS.JOB_POSTED_DATE
    ) {
      value = toDateInputValue(raw) || String(raw ?? '').trim();
    } else {
      value = hyperlinkLabel(raw);
    }
    const next = value == null ? '' : String(value);
    inputs[`col_${col}`] = next;
    if (tag === FIELD_TAGS.CUSTOM_DROPDOWN) {
      inputs[`dropdown_${col}`] = next;
    } else if (
      tag === FIELD_TAGS.CUSTOM_TEXT ||
      tag === FIELD_TAGS.CUSTOM_TEXTBOX
    ) {
      inputs[`text_${col}`] = next;
    } else if (tag) {
      inputs[tag] = next;
    }
  }
 ...
 */
export function getAutoValueForTag(tag, extractedData = {}) {
  switch (tag) {
    case FIELD_TAGS.DATE_APPLIED:
    case FIELD_TAGS.CURRENT_DATE:
      return formatDate();
    ...
    case FIELD_TAGS.JOB_POSTED_DATE:
      return toDateInputValue(extractedData.postedDate) || extractedData.postedDate || '';
    case FIELD_TAGS.APPLICATION_STATUS:
      return extractedData.applicationStatus || 'Applied';
    default:
      return '';
  }
}
 ...
 */
export function getFieldsNeedingInput(mappings, extractedData) {
  const needsInput = [];
  ...
  for (const mapping of mappings) {
    ...
    if (tag === FIELD_TAGS.CUSTOM_DROPDOWN) {
      ...
      const fallbackStatus = extractedData.applicationStatus || 'Applied';
    ...
    const autoValue = getAutoValueForTag(tag, extractedData);
